...
class HrExpenseSheet(models.Model):
    ...
    @api.depends("account_move_ids.amount_residual")
    def _compute_amount_residual(self):
        for sheet in self:
            sheet.amount_residual = sum(
                sheet.account_move_ids.mapped("amount_residual")
            )
    ...
    def _compute_state(self):
        for sheet in self:
            ...
            if not expenses:
                ...
                sheet.state = "paid"
            elif all(expense.state in ("posted", "paid") for expense in expenses):
                sheet.state = "posted"
    ...
    @api.depends("expense_line_ids.account_move_id.payment_state")
    def _compute_payment_state(self):
        for sheet in self:
            moves = sheet.expense_line_ids.account_move_id
            if sheet.payment_mode == "company_account":
                if moves.filtered(lambda move: move.state != "draft"):
                    # when the sheet is paid by the company, the state/amount of the
                    # related account_move_ids are not relevant unless all moves have
                    # been reversed
                    if moves - moves.filtered("reversal_move_ids"):
                        sheet.payment_state = "paid"
                    else:
                        sheet.payment_state = "reversed"
                else:
                    payment_states = set(moves.mapped("payment_state"))
                    if len(payment_states) <= 1:  # If only 1 move or only one state
                        sheet.payment_state = (
                            payment_states.pop() if payment_states else "not_paid"
                        )
                    elif (
                        "partial" in payment_states or "paid" in payment_states
                    ):  # else if any are (partially) paid
                ...
                if moves.filtered(lambda move: move.state == "posted"):
                    sheet.payment_state = moves[:1].payment_state
                else:
                    sheet.payment_state = "not_paid"
    ...
    def action_open_expense_view(self):
        self.ensure_one()
        return self.expense_line_ids._get_records_action(name=self.env._("Expenses"))
